...
// Returns { sunrise, sunset } as Unix timestamps for a given lat/lon and date string (YYYY-MM-DD).
// Uses the NOAA solar calculation algorithm. Returns null if polar night/day.
export function getSunTimes(lat, lon, dateStr) {
  ...
  const lambda = (sunLon - 0.00569 - 0.00478 * Math.sin(omega * Math.PI / 180)) * Math.PI / 180;
  const epsilon0 = 23 + (26 + (21.448 - T * (46.815 + T * (0.00059 - T * 0.001813))) / 60) / 60;
  const epsilon = (epsilon0 + 0.00256 * Math.cos(omega * Math.PI / 180)) * Math.PI / 180;
  ...
  const EqT = 4 * (180 / Math.PI) * (
    y * Math.sin(2 * L0r) - 2 * e * Math.sin(Mrad) + 4 * e * y * Math.sin(Mrad) * Math.cos(2 * L0r)
  ...
  const solarNoonFrac = (720 - 4 * lon - EqT) / 1440;
  const dayStartUnix = Math.floor(new Date(`${dateStr}T00:00:00Z`).getTime() / 1000);
  return {
    sunrise: dayStartUnix + Math.round((solarNoonFrac - HA * 4 / 1440) * 86400),
    sunset:  dayStartUnix + Math.round((solarNoonFrac + HA * 4 / 1440) * 86400),
  };
}
